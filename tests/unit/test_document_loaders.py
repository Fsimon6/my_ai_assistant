...
# 这些加载器依赖**可选**的重型库（PyMuPDF(fitz) / Pillow / python-docx 等），
# 它们不在 backend/requirements.txt 的运行依赖内（见该文件末尾说明）。
# 未安装时整体跳过本模块，避免全新克隆后 `pytest tests/unit` 在**收集阶段**直接报错。
pytest.importorskip('fitz', reason='未安装 PyMuPDF：backend/document_loaders 属可选能力')
...
from backend.document_loaders import DocumentLoader
